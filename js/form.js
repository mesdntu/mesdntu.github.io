...
function nextStep(nextId, isYes, advice) {
    const currentSection = document.querySelector('.question:not([style*="display: none"]), .info:not([style*="display: none"])');
    if (currentSection) {
...
function saveAdvice(advice) {
    if (advice) {
        userAdvice.push(advice);
    }
}

function showSummary() {
    const allSections = document.querySelectorAll('.question, .info');
    allSections.forEach(section => section.style.display = 'none');

    const summarySection = document.getElementById('summary');
    const adviceList = document.getElementById('adviceList');
    adviceList.innerHTML = userAdvice.map(advice => `<li>${advice}</li>`).join('');
