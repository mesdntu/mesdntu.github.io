...
async function loadVesselDataForOwner(ownerName) {
    try {
        ...
        const vesselText = vesselCount === 1 ? `${vesselCount} Vessel` : `${vesselCount} Vessels`;
        document.getElementById('vessel_count').textContent = vesselText;
...
function redirectToVesselPage(vesselName) {
    const ownerName = document.getElementById('owner_name').textContent; // 获取当前 owner 名称
    window.location.href = `/pages/vessel.html?vessel=${encodeURIComponent(vesselName)}&owner=${encodeURIComponent(ownerName)}`;
}
